...
export const ToastItem: React.FC<ToastItemProps> = ({ type, message, onClose }) => {
    ...
    return (
        ...
        >
            <div
                className={`inline-flex items-center justify-center shrink-0 w-8 h-8 ${config.iconColor} ${config.bgIcon} rounded-lg`}
            >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                    {config.iconPath}
                </svg>
            </div>
            <div className="ms-3 text-sm font-normal mr-8">{message}</div>
            <button
                type="button"
                onClick={onClose}

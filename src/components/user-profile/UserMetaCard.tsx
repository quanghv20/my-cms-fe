...
import { ISocial } from '@/type/profile.type';
...
import EditMetaProfileModal from './EditMetaProfileModal';
...
export default function UserMetaCard({ profile }: PropsType) {
  ...
  return (
    <>
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            ...
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              {profile.socials?.map((item: ISocial) => (
                <a
                  key={item._id}
                  href={item.link}
                  target="_blank"
                ...
                >
                  <span className="fill-current text-gray-700 dark:text-gray-300">
        ...
        <EditMetaProfileModal isOpen={isOpen} onClose={closeModal} profile={profile} />
      </div>
    </>
  );
}
